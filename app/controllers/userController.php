				...
				foreach($datos as $rows){
					$tabla.='
						<tr class="" >
							...
							<!--<td>
			                    <a href="'.APP_URL.'userPhoto/'.$rows['usuario_id'].'/" class="btn btn-dark">Foto</a>
			                </td>-->
			                <td>
			                    <a href="'.APP_URL.'userUpdate/'.$rows['usuario_id'].'/" class="btn btn-success"><i class="bi bi-pencil-square"></i> Editar</a>
			                </td>
			                <td>
		...
		public function actualizarUsuarioControlador(){
		    ...
		    if($admin_usuario=="" || $admin_clave==""){
		        $alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
					"texto"=>"No ha llenado todos los campos que son obligatorios, que corresponden a su RUT y CLAVE",
					"icono"=>"error"
				];
				return json_encode($alerta);
		        exit();
		    ...
		    if($check_admin->rowCount()==1){
		    	...
		    	if($check_admin['usuario_usuario']!=$admin_usuario || !password_verify($admin_clave,$check_admin['usuario_clave'])){
		    		...
		    		$alerta=[
						"tipo"=>"simple",
						"titulo"=>"Ocurrió un error inesperado",
						"texto"=>"RUT o CLAVE de administrador incorrectos",
						"icono"=>"error"
					];
					return json_encode($alerta);
		        	exit();
